...
ALTBASE_UTXO_SERVICE_API char* ALTBASE_UTXO_SERVICE_CALL ALTBASE_UTXO_SERVICE_REQUEST(const char* request) {
  ...
  try {
    ...
    return copy_response(altbase::ok_response(id, altbase::handle_utxo_tx(*parsed)));
  } catch (const std::exception& error) {
    return copy_response(altbase::error_response(id, "utxo_tx_error", error.what()));
  } catch (...) {
    return copy_response(altbase::error_response(id, "utxo_tx_error", "unknown UTXO transaction failure"));
  }
}
